...
export default async (req) => {
  ...
  try {
    ...
    const { data: invite, error } = await supabase
      ...
      .select("id, code, status, participant:competition_participants!competition_invite_codes_participant_id_fkey(id, display_name, invite_code_id)")
      .eq("code", code)
      .maybeSingle();

    if (error) throw error;
    if (!invite) return json({ participant: null, codeStatus: "unknown" }, 404);
    return json({ participant: invite.participant ?? null, codeStatus: invite.status, code: invite.code });
  } catch (error) {
